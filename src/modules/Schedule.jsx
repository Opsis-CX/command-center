    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 20, marginBottom: 18, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          {canFillRate && (
            <button className={'btn ' + (tab === 'hourly' ? 'btn-primary' : 'btn-ghost')} onClick={() => setTab('hourly')}>Hourly Report</button>
          )}
        </div>
      </div>
...
// ============================================================
// Hourly QA Report — auto-fills the two tracker sheets (Affiliate
// Leads + Open Invoices) from BigQuery so the ASC only types
// commentary, then one-click posts to #GarageCo Reporting.
// ============================================================
// QA quality colour: target is 90+. Green ≥90, amber 80–90, red <80.
function qaColor(v) { return v == null ? 'var(--ink)' : v >= 90 ? '#1b5e20' : v >= 80 ? '#8d6e00' : '#b71c1c' }

function HourlyReportView({ me }) {
  const [day, setDay] = useState(() => isoDate(etNow()))
  const [data, setData] = useState(null)
  const [loading, setLoading] = useState(true)
  ...
  const load = useCallback(async () => {
    setLoading(true); setErr('')
    const { data, error } = await supabase.rpc('get_asc_hourly_report', { p_day: day })
    if (error) { setErr(error.message); setData(null) } else { setData(data) }
    setLoading(false)
  }, [day])
  useEffect(() => { load() }, [load])

  // Pull the latest rows straight from the tracker sheets, then reload.
  async function refreshFromSheets() {
    setSyncing(true); setErr('')
    try {
      const { error } = await supabase.rpc('refresh_qa_trackers')
  ...
  const posted = data?.posted_reports || []
  ...
  function trackerLine(t) {
    const avg = t.avg_score == null ? 'n/a' : t.avg_score + '%'
    const pr = t.pass_rate == null ? 'n/a' : t.pass_rate + '%'
    return `${t.label}: ${t.audited} audited · avg ${avg} · ${pr} at target · ${t.auto_fails} auto-fail · ${t.dispo_fixes} dispo fix`
  }
  function buildUpdate() {
    if (!data) return ''
    const lines = [
      `GarageCo Hourly QA Update — ${dayLabel}${data.is_today ? ` · as of ${nowEt}` : ''}`,
      ``,
      ...trackers.map(trackerLine),
      ``,
    ]
    if (where.trim()) lines.push(`Where we are: ${where.trim()}`)
    if (work.trim()) lines.push(`Working on: ${work.trim()}`)
    if (actions.trim()) lines.push(`Actions: ${actions.trim()}`)
    if (where.trim() || work.trim() || actions.trim()) lines.push(``)
  ...
  async function saveReport() {
    setErr('')
    const { error } = await supabase.rpc('post_asc_hourly_report', {
      p_snapshot: { trackers, day: data?.day, generated_at: data?.generated_at },
      p_where: where.trim() || null, p_work: work.trim() || null, p_actions: actions.trim() || null,
      p_formatted: buildUpdate(),
    })
    if (error) { setErr(error.message); return }
    setSaved(true); setTimeout(() => setSaved(false), 2000)
    setWhere(''); setWork(''); setActions('')
    load()
  }

  if (loading) return <p className="page-sub">Loading hourly report…</p>
  if (err && !data) return <div className="card" style={{ borderColor: 'var(--failed)' }}><b style={{ color: 'var(--failed)' }}>Couldn't load hourly report.</b><p className="page-sub" style={{ marginTop: 6 }}>{err}</p></div>
  if (!data) return null
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: 14 }}>
        <input type="date" value={day} onChange={e => setDay(e.target.value)}
          style={{ padding: '8px 10px', border: '1px solid var(--line)', borderRadius: 8, fontSize: 13, fontFamily: 'inherit', background: 'var(--canvas)' }} />
        <button className="btn btn-ghost" onClick={refreshFromSheets} disabled={syncing}>{syncing ? '⏳ Syncing…' : '↻ Refresh from sheets'}</button>
        <span className="page-sub" style={{ fontSize: 12 }}>synced {syncedLabel}{data.last_audit_at ? ` · last audit ${new Date(data.last_audit_at).toLocaleTimeString('en-US', { timeZone: COMPANY_TZ, hour: 'numeric', minute: '2-digit' })}` : ''}</span>
        <button className="btn btn-ghost" style={{ marginLeft: 'auto' }} onClick={copyUpdate}>{copied ? '✓ Copied' : '📋 Copy update'}</button>
        <button className="btn btn-primary" onClick={saveReport}>{saved ? '✓ Saved' : 'Save update'}</button>
      </div>
      ...
      {/* Per-tracker snapshot */}
      {trackers.map(t => (
        <div key={t.key} style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 13.5, fontWeight: 800, marginBottom: 8 }}>{t.label} <span style={{ color: 'var(--ink-soft)', fontWeight: 500, fontSize: 12.5 }}>· {t.audited} audited today by {t.agents} agent{t.agents === 1 ? '' : 's'}</span></div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 12, marginBottom: 12 }}>
            <StatCard label="Audited" big={String(t.audited)} sub="calls scored" />
            <StatCard label="Avg Score" big={t.avg_score == null ? '—' : t.avg_score + '%'} bigColor={qaColor(t.avg_score)} sub="target 90+" />
            <StatCard label="At Target" big={t.pass_rate == null ? '—' : t.pass_rate + '%'} bigColor={qaColor(t.pass_rate)} sub={`${t.passes} of ${t.audited} ≥90`} />
            <StatCard label="Auto-fails" big={String(t.auto_fails)} bigColor={t.auto_fails > 0 ? '#b71c1c' : '#1b5e20'} sub="critical misses" />
            <StatCard label="Dispo Fixes" big={String(t.dispo_fixes)} bigColor={t.dispo_fixes > 0 ? '#8d6e00' : '#1b5e20'} sub="wrong disposition" />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14 }}>
            <div className="card">
              <div style={{ fontSize: 12.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.04em', color: 'var(--accent)', marginBottom: 10 }}>By Brand</div>
              {(t.by_brand || []).length === 0 ? <p className="page-sub" style={{ fontSize: 13, margin: 0 }}>No audits yet today.</p> : (t.by_brand || []).map((b, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0', borderTop: i ? '1px solid var(--line-soft)' : 'none', fontSize: 13.5 }}>
                  <span style={{ fontWeight: 600 }}>{b.brand}</span>
                  <span><span style={{ color: 'var(--ink-soft)', fontSize: 12 }}>{b.n} audit{b.n === 1 ? '' : 's'} · </span><b style={{ color: qaColor(b.avg) }}>{b.avg == null ? '—' : b.avg + '%'}</b></span>
                </div>
              ))}
            </div>
            <div className="card">
              <div style={{ fontSize: 12.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.04em', color: 'var(--accent)', marginBottom: 10 }}>Needs Attention</div>
              {(t.low_scores || []).length === 0 ? <p className="page-sub" style={{ fontSize: 13, margin: 0 }}>No misses — everyone at target. 🎉</p> : (t.low_scores || []).map((r, i) => (
                <div key={i} style={{ padding: '7px 0', borderTop: i ? '1px solid var(--line-soft)' : 'none', fontSize: 13 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontWeight: 600 }}>{r.agent} <span style={{ color: 'var(--ink-soft)', fontWeight: 400, fontSize: 12 }}>· {r.brand}</span></span>
                    <span>{r.auto_fail && <span style={{ fontSize: 11, fontWeight: 700, padding: '1px 7px', borderRadius: 999, background: '#fdecea', color: '#b71c1c', marginRight: 6 }}>AUTO-FAIL</span>}<b style={{ color: qaColor(r.score) }}>{r.score == null ? '—' : r.score + '%'}</b></span>
                  </div>
                  {r.correct && r.cur && r.correct.trim().toLowerCase() !== (r.cur || '').trim().toLowerCase() && (
                    <div style={{ fontSize: 11.5, color: '#8d6e00', marginTop: 2 }}>dispo: {r.cur || '—'} → {r.correct}</div>
                  )}
                  {r.note && <div style={{ fontSize: 11.5, color: 'var(--ink-soft)', marginTop: 2 }}>{r.note}</div>}
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Commentary */}
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 12.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.04em', color: 'var(--accent)', marginBottom: 10 }}>Your Commentary</div>
        <div style={{ display: 'grid', gap: 10 }}>
          <label style={{ fontSize: 12.5 }}><span style={{ fontWeight: 600 }}>Where we are</span>
            <textarea value={where} onChange={e => setWhere(e.target.value)} rows={2} placeholder="Where the numbers stand right now…" style={taStyle} /></label>
          <label style={{ fontSize: 12.5 }}><span style={{ fontWeight: 600 }}>What we're working on</span>
            <textarea value={work} onChange={e => setWork(e.target.value)} rows={2} placeholder="The issues you're focused on…" style={taStyle} /></label>
          <label style={{ fontSize: 12.5 }}><span style={{ fontWeight: 600 }}>Actions to improve</span>
            <textarea value={actions} onChange={e => setActions(e.target.value)} rows={2} placeholder="What you're doing to move the numbers…" style={taStyle} /></label>
        </div>
      </div>

      {/* Preview */}
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 12.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.04em', color: 'var(--accent)', marginBottom: 10 }}>Post Preview</div>
      ...
      {posted.length > 0 && (
        <div className="card">
          <div style={{ fontSize: 12.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.04em', color: 'var(--accent)', marginBottom: 10 }}>Posted {dayLabel}</div>
          {posted.map((p, i) => (
            <div key={p.id} style={{ padding: '8px 0', borderTop: i ? '1px solid var(--line-soft)' : 'none', fontSize: 13 }}>
              <div style={{ fontWeight: 600 }}>{new Date(p.at).toLocaleTimeString('en-US', { timeZone: COMPANY_TZ, hour: 'numeric', minute: '2-digit' })} <span style={{ color: 'var(--ink-soft)', fontWeight: 400, fontSize: 12 }}>· {p.author || 'ASC'}</span></div>
              {p.where_we_are && <div style={{ marginTop: 2 }}><span style={{ color: 'var(--ink-soft)' }}>Where: </span>{p.where_we_are}</div>}
              {p.what_to_work_on && <div><span style={{ color: 'var(--ink-soft)' }}>Working on: </span>{p.what_to_work_on}</div>}
              {p.actions && <div><span style={{ color: 'var(--ink-soft)' }}>Actions: </span>{p.actions}</div>}
            </div>
          ))}
        </div>
      )}
      ...
      <p className="page-sub" style={{ fontSize: 11.5, marginTop: 12 }}>
        Numbers auto-fill from the Affiliate Leads &amp; Open Invoices QA tracker sheets (BigQuery). They sync hourly; hit “Refresh from sheets” to pull the latest before you post. Add your commentary, then Copy update to paste into #GarageCo Reporting or Save to log it here.
      </p>
    </div>
  )
}
