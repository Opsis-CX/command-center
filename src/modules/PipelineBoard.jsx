...
//
// Two stage sets share this board (see PipelineBoard's `stages` prop):
...
const STAGE_LABEL = {
  ...
  call_2_made: 'Call 2 Made', linkedin_2_sent: 'LinkedIn Message 2 Sent', linkedin_3_sent: 'LinkedIn Message 3 Sent',
  drip_campaign: 'Added to Drip Campaign',
...
export default function PipelineBoard({ heading = 'Sales pipeline', stages: STAGES = SALES_STAGES, pipelineKey = 'sales' }) {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error } = await supabase.from('deals')
      .select('*').eq('pipeline', pipelineKey).order('created_at', { ascending: false })
  ...
  useEffect(() => {
    const ch = supabase.channel('pipeline-' + pipelineKey)
