...
function NewAudit({ prefill, editAudit, onDone }) {
  ...
  // ---- Auto-save draft: only on the plain "New audit" tab (not the queue
  // prefill flow, and not when correcting a submitted audit). Read once. ----
  const draftEnabled = !editAudit && !prefill
  const draftKey = draftEnabled ? ('qa_audit_draft_' + (user?.id || 'anon')) : null
  ...
  const [campaign, setCampaign] = useState(editAudit?.campaign || initialDraft?.campaign || 'lavin')
  const [auditType, setAuditType] = useState(editAudit?.audit_type || initialDraft?.auditType || 'conversation')
  const [agentName, setAgentName] = useState(editAudit?.agent_name || prefill?.agent_name || initialDraft?.agentName || '')
  const [callId, setCallId] = useState(editAudit?.call_id || prefill?.call_id || initialDraft?.callId || '')
  const [callDate, setCallDate] = useState(editAudit?.call_date || prefill?.call_date || initialDraft?.callDate || new Date().toISOString().slice(0, 10))
  const [recording, setRecording] = useState(editAudit?.recording_link || prefill?.recording_link || initialDraft?.recording || '')
  const [uploadingRec, setUploadingRec] = useState(false)
  const [recError, setRecError] = useState('')
  const [brand, setBrand] = useState(editAudit?.brand || prefill?.brand || initialDraft?.brand || '')
  const [answers, setAnswers] = useState(editAudit?.answers || initialDraft?.answers || {})  // { question_id: { value:'yes'|'no'|'na', missed:[subItemId] } }
  const [autoFail, setAutoFail] = useState(editAudit ? !!editAudit.auto_fail : !!initialDraft?.autoFail)
  const [curDisp, setCurDisp] = useState(editAudit?.current_disposition || prefill?.disposition || initialDraft?.curDisp || '')
  const [correctDisp, setCorrectDisp] = useState(editAudit?.correct_disposition || initialDraft?.correctDisp || '')
  const [feedback, setFeedback] = useState(editAudit?.feedback || initialDraft?.feedback || '')
  const [draftNotice, setDraftNotice] = useState(!!(draftEnabled && initialDraft && ((initialDraft.answers && Object.keys(initialDraft.answers).length) || initialDraft.feedback || initialDraft.callId || initialDraft.recording || initialDraft.curDisp || initialDraft.correctDisp)))
  ...
  // Persist the in-progress audit to this device so nothing is lost if the
  // auditor clicks to another tab or the page reloads. Cleared on submit/discard.
  const clearDraft = useCallback(() => { try { if (draftKey) window.localStorage.removeItem(draftKey) } catch {} }, [draftKey])
  const draftHydrated = useRef(false)
  useEffect(() => {
    if (!draftEnabled || !draftKey) return
    if (!draftHydrated.current) { draftHydrated.current = true; return }  // don't re-save the value we just restored
    const hasContent = (answers && Object.keys(answers).length) || !!feedback || !!callId || !!recording || !!curDisp || !!correctDisp
    try {
      if (hasContent) window.localStorage.setItem(draftKey, JSON.stringify({ campaign, auditType, agentName, callId, callDate, recording, brand, answers, autoFail, curDisp, correctDisp, feedback, savedAt: Date.now() }))
      else window.localStorage.removeItem(draftKey)
    } catch {}
  }, [draftEnabled, draftKey, campaign, auditType, agentName, callId, callDate, recording, brand, answers, autoFail, curDisp, correctDisp, feedback])

  function discardDraft() {
    clearDraft(); setDraftNotice(false)
    setCampaign('lavin'); setAuditType('conversation'); setAgentName(''); setCallId('')
    setCallDate(new Date().toISOString().slice(0, 10)); setRecording(''); setBrand('')
    setAnswers({}); setAutoFail(false); setCurDisp(''); setCorrectDisp(''); setFeedback('')
  }
  ...
  return (
    <div style={{ maxWidth: 880 }}>
      ...
      {draftNotice && (
        <div className="card" style={{ borderColor: 'var(--accent)', marginBottom: 14, display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          <span style={{ fontSize: 13.5 }}>📝 <b>Draft restored.</b> <span className="page-sub">We kept your unsaved audit on this device{initialDraft?.savedAt ? ' (' + new Date(initialDraft.savedAt).toLocaleString() + ')' : ''}.</span></span>
          <button className="btn btn-ghost" style={{ marginLeft: 'auto', fontSize: 12, padding: '4px 10px' }} onClick={discardDraft}>Discard draft &amp; start fresh</button>
        </div>
      )}
      {editAudit && (
      ...
      <div className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div className="page-sub" style={{ fontSize: 12.5 }}>
            ...
            {draftEnabled && <span style={{ display: 'block', fontSize: 11.5, opacity: 0.8 }}>Progress auto-saved on this device until you submit</span>}
          </div>
        </div>
