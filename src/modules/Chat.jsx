...
function ChannelPane({ channelId, me, isAdmin, isOwner, channel, dmName, profiles, isMobile, onBack, markRead }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0, position: 'relative' }}>
      ...
      <div style={{ borderTop: '1px solid var(--line)', padding: isMobile ? '8px 10px' : '10px 16px', flex: 'none', background: 'var(--surface)' }}>
        {isAdmin && (
          <label style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 8, fontSize: 12.5, color: requireAck ? 'var(--accent)' : 'var(--ink-soft)', cursor: 'pointer', fontWeight: requireAck ? 600 : 400 }}>
            ...
            {isMobile
              ? <span>Post as <b>@update</b></span>
              : <span>Post as <b>@update</b> — require everyone to confirm they've read it</span>}
          </label>
        )}
        {pending.length > 0 && (
        ...
        <div style={{ display: 'flex', gap: 8, position: 'relative', alignItems: 'flex-start', flexDirection: isMobile ? 'column' : 'row' }}>
          {showPicker && (
          ...
          {/* Desktop: attach + emoji sit to the LEFT of the composer */}
          {!isMobile && (
          ...
          {isMobile ? (
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', width: '100%' }}>
              <button type="button" onClick={() => fileInputRef.current?.click()} title="Attach file"
                style={{ border: '1px solid var(--line)', background: 'var(--surface)', borderRadius: 8, cursor: 'pointer', fontSize: 17, padding: '9px 13px', flex: 'none' }}>📎</button>
              <button type="button" onClick={() => setShowPicker(p => !p)} title="Emoji"
                style={{ border: '1px solid var(--line)', background: 'var(--surface)', borderRadius: 8, cursor: 'pointer', fontSize: 18, padding: '9px 13px', flex: 'none' }}>😀</button>
              <button className={'btn ' + (requireAck ? 'btn-cta' : 'btn-primary')} style={{ flex: 1 }} onClick={send} disabled={uploading}>{requireAck ? 'Post update' : 'Send'}</button>
            </div>
          ) : (
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'flex-start' }}>
        {toolbarCollapsible && (
          <button type="button" title={toolbarOpen ? 'Hide formatting' : 'Formatting'}
            ...
            style={{ border: 0, background: 'transparent', cursor: 'pointer', color: toolbarOpen ? 'var(--accent)' : 'var(--ink-soft)', fontSize: 14, fontWeight: 700, padding: '11px 6px 0 10px', flex: 'none', fontFamily: 'inherit', lineHeight: 1 }}>Aa</button>
        )}
