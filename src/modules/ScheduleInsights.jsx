  ...
  return (
    <>
      ...
      {Object.keys(byClient).sort().map(clientName => {
        ...
        // per-ROLE fill rate within this client (agents vs ASC vs each position)
        const roleAgg = {}
        cBlocks.forEach(b => {
          const rk = b.role || positionForBlock(b) || 'Unspecified'
          const a = (roleAgg[rk] = roleAgg[rk] || { total: 0, filled: 0 })
          a.total += b.total_spots
          a.filled += claimsFor(b.id).length
        })
        const roleRows = Object.entries(roleAgg).sort((a, b) => a[0].localeCompare(b[0]))
        // group by hour
        const byHour = {}
        ...
        return (
          <div className="card" key={clientName} style={{ marginBottom: 14 }}>
            ...
            {roleRows.length > 1 && (
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginBottom: 10, paddingBottom: 10, borderBottom: '1px solid var(--line-soft)' }}>
                <span className="page-sub" style={{ fontSize: 12, fontWeight: 600 }}>By role:</span>
                {roleRows.map(([rk, a]) => {
                  const p = a.total ? Math.round((a.filled / a.total) * 100) : 0
                  return <span key={rk} className="badge" style={{ background: p >= 95 ? 'var(--passed-bg)' : p >= 90 ? 'var(--needed-bg)' : 'var(--failed-bg)', color: p >= 95 ? 'var(--passed)' : p >= 90 ? 'var(--needed)' : 'var(--failed)' }}>{rk} — {a.filled}/{a.total} ({p}%)</span>
                })}
              </div>
            )}
